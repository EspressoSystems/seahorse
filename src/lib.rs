        ...
        'a: 'b,
    {
        async move {
            ...
            if policy.is_auditor_pub_key_set() {
                if let Some(account) = self.viewing_accounts.get_mut(policy.auditor_pub_key()) {
                    if !account.used {
                        account.used = true;
                        used_viewing_key = Some(policy.auditor_pub_key());
                    }
                }
            }
    ...
    Wallet<'a, Backend, L>
{
    ...
    pub async fn viewing_account(
        ...
        let records = state
            ...
            .iter()
            .filter(|rec| {
                rec.ro.asset_def.policy_ref().auditor_pub_key() == address
    ...
    pub async fn transfer(
        ...
        asset: &AssetCode,
        receivers: &[(UserAddress, impl Clone + Into<RecordAmount>)],
    ...
    pub async fn build_transfer(
        ...
        sender: Option<&UserAddress>,
        asset: &AssetCode,
        receivers: &[(UserAddress, impl Clone + Into<RecordAmount>, bool)],
        fee: impl Into<RecordAmount>,
        bound_data: Vec<u8>,
