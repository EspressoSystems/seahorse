...
use crate::{
    ...
    records::{Record, Records},KeystoreModel,KeystoreBackend,EventSummary,key_scan::BackgroundKeyScan,
    sparse_merkle_tree::SparseMerkleTree,Transactions,TransactionUID,
    transactions::{SignedMemos, TransactionParams},RecordAmount,input_records,receive_history_entry,CommittedTxn,Captures,MintInfo,
    txn_builder::{TransferSpec,TransactionStatus},TransactionError,EncryptingResourceAdapter,KeystoreError,CryptoSnafu
};use std::sync::Arc;use jf_cap::{MerkleTree};
use arbitrary::{Arbitrary, Unstructured};
use ark_serialize::*;
use chrono::Local;use std::collections::HashMap;
use derive_more::*;use std::iter::repeat;
use espresso_macros::ser_test; use atomic_store::RollingLog;
use jf_cap::{
    errors::TxnApiError,MerklePath,
    freeze::{FreezeNote, FreezeNoteInput},
    keys::{FreezerKeyPair, FreezerPubKey, ViewerKeyPair,UserAddress, UserKeyPair, UserPubKey, ViewerPubKey},
    mint::MintNote,
...
use jf_utils::tagged_blob;use ark_std::future::Future;
use key_set::{KeySet,ProverKeySet};
use num_bigint::{BigInt, Sign};
use num_traits::identities::{One, Zero};
use primitive_types::U256;
use rand_chacha::ChaChaRng; use futures::future::BoxFuture;use futures::stream::Stream;
#[cfg(test)]
use reef::cap;
use reef::{
    traits::{
        Block as _, NullifierSet as _, Transaction , TransactionKind as _,
...
impl<'a, L: 'static + Ledger> LedgerState<'a, L> {
    ...
    pub async fn handle_event<Meta: Serialize + DeserializeOwned + Send>(
        &mut self,
  ...
  pub  async fn import_memo<Meta: Serialize + DeserializeOwned + Send>(
   ...
   pub  fn define_asset<'b, Meta: Serialize + DeserializeOwned + Send + Send>(
    ...
    {
        async move {
            let seed = AssetCodeSeed::generate(model.rng);
   ...
   pub  async fn submit_transaction<Meta: Serialize + DeserializeOwned + Send>(
    ...
    pub async fn submit_elaborated_transaction<'b, Meta: Serialize + DeserializeOwned + Send + Send>(
        &'b mut self,
        model: &'b mut KeystoreModel<'a, L, impl KeystoreBackend<'a, L>, Meta>,
        txn: reef::Transaction<L>,
...
pub struct LedgerStateStore<'a, L:Ledger+DeserializeOwned+Serialize> {
    store: RollingLog::<EncryptingResourceAdapter<LedgerState<'a, L>>>
}
...
impl<'a, L:Ledger+DeserializeOwned+Serialize> LedgerStateStore<'a,L> {
    ...
    pub fn update(&mut self, new_state: &LedgerState<'a, L>) -> Result<(),KeystoreError<L>> {
       Ok(self.store.store_resource(new_state)?)
    }
}
