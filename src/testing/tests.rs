...
pub mod generic_wallet_tests {
    ...
    pub async fn test_zero_fee<'a, T: SystemUnderTest<'a>>() {
        ...
        let (ledger, mut wallets) = t.create_test_network(&[(4, 4)], vec![2], &mut now).await;
        ledger.lock().await.set_block_size(1).unwrap();

        // Allocating a zero fee is tricky if the wallet has one account with 0 native tokens and
        ...
        // panic. To test with these conditions, we will create a fresh wallet with two accounts,
        // and fund only the second one.
        let mut sender = t.create_wallet(&mut rng, &ledger).await;
        sender
        ...
        let txn = wallets[0]
            .0
            .transfer(None, &AssetCode::native(), &[(accounts[1].address(), 1)], 0)
            .await
            .unwrap();
        await_transaction(&txn, &wallets[0].0, &[&sender]).await;
        assert_eq!(
            sender
        ...
        let asset = sender
            ...
            .unwrap();
        let txn = sender
            .mint(None, 0, &asset.code, 1, accounts[0].address())
            .await
            .unwrap();
        ...
        let txn = sender
            .transfer(None, &asset.code, &[(wallets[0].1[0].clone(), 1)], 0)
            .await
            .unwrap();
        await_transaction(&txn, &sender, &[&wallets[0].0]).await;
        assert_eq!(sender.balance(&AssetCode::native()).await, 1u64.into());
        assert_eq!(sender.balance(&asset.code).await, 0u64.into());
        assert_eq!(wallets[0].0.balance(&asset.code).await, 1u64.into());
    }
}
