...
pub mod generic_keystore_tests {
    ...
    async fn test_two_keystores<'a, T: SystemUnderTest<'a>>(native: bool) {
        ...
        let (ledger, mut keystores) = t
            ...
            .await;
        let alice_pub_keys = keystores[0].1;
        let alice_addresses = alice_pub_keys.clone().address();
        let bob_pub_keys = keystores[1].1;
        let bob_addresses = bob_pub_keys.clone().address();
    ...
    pub async fn test_keystore_freeze<'a, T: SystemUnderTest<'a>>() -> std::io::Result<()> {
        ...
        assert_eq!(
            keystores[0]
                .0
                .balance_breakdown(&keystores[0].1[0].balance(), &asset.code)
                .await,
            0
        );
        assert_eq!(
            keystores[0]
                .0
                .frozen_balance_breakdown(&keystores[0].1[0].balance(), &asset.code)
                .await,
            0
        );
        assert_eq!(
            keystores[1]
                .0
                .balance_breakdown(&keystores[1].1[0].balance(), &asset.code)
                .await,
            1
        );
    ...
    async fn test_multixfr_keystore<'a, T: SystemUnderTest<'a>>(
        ...
        for (i, block) in txs.iter().enumerate() {
            ...
            for (j, (asset_ix, sender_ix, receiver_ix, amount)) in block.iter().enumerate() {
                ...
                let sender_address = keystores[sender_ix + 1].1[0].clone().address();
    ...
    pub async fn test_generate_user_key<'a, T: SystemUnderTest<'a>>() {
        ...
        assert_eq!(
            keystores[0]
                .0
                .balance_breakdown(&key.pub_key(), &AssetCode::native())
                .await,
            0
        );
    ...
    pub async fn test_create_with_existing_ledger<'a, T: SystemUnderTest<'a>>() {
        ...
        assert_eq!(
            keystore2
                .balance_breakdown(&pub_keys2[0].address(), &AssetCode::native())
                .await,
            2
        );
    ...
    pub async fn test_aggregate_addresses<'a, T: SystemUnderTest<'a>>() {
        ...
        let (ledger, mut keystores) = t
            ...
            .await;
        let alice_addresses = keystores[0].1.clone().address();
        let bob_pub_keys = keystores[1].1.clone();
        let bob_addresses = bob_pub_keys.address();
