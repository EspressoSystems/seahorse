...
pub trait MockNetwork<L: Ledger> {
    ...
    /// Submit a block.
    ///
    /// Returns the expected number of [Memos](LedgerEvent::Memos) events to be generated by this
    /// block.
    fn submit(&mut self, block: Block<L>) -> Result<usize, KeystoreError<L>>;
    fn post_memos(
        &mut self,
        block_id: u64,
        txn_id: u64,
...
impl<L: Ledger, N: MockNetwork<L>> MockLedger<L, N> {
    ...
    pub fn flush(&mut self) -> Result<(), KeystoreError<L>> {
        ...
        let txns = std::mem::take(&mut self.current_block);
        let mut block = self.network().state().next_block();
        for txn in txns {
            block.add_transaction(txn).unwrap();
        }
        self.missing_memos += self.network.submit(block)?;
        Ok(())
    }
    ...
    pub fn submit(&mut self, txn: reef::Transaction<L>) -> Result<(), KeystoreError<L>> {
        if self.hold_next_transaction {
            ...
            let mut rejected = self.network.state().next_block();
            rejected.add_transaction(txn).unwrap();
...
>(
    ...
    receivers: &[&Keystore<Backend, L, Meta>],
) {
    assert_eq!(
